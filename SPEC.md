...
- 初期エリア: `tsukiji-akashicho`。bbox は緯度 35.658–35.674 / 経度 139.764–139.7815(loop_001 で確定。
  東端は隅田川中心線の近似とし、佃・月島 = 将来エリア `tsukuda-tsukishima` の地物混入を防ぐ。
  bbox は粗い抽出フィルタ + Q-02 検証用で、エリア帰属の正は各レコードの area 列)
- 対象行政区: 中央区(東京都)= Wikidata **Q212704**(loop_001 で実照会により確定。
  同名区が多数あるためラベル一致での特定は禁止)
...
F-01 注記(loop_001 実データ確認済み): 抽出は `wdt:P131+ wd:Q212704`(推移的)を用いる
(P131 は町丁粒度が多く、直付けでは築地本願寺・聖路加等を取りこぼす)。Wikidata 上の実態は
寺社・橋・教会クラス + P1435(文化財指定)が主で、記念碑・跡地系クラスはほぼ 0 件のため、
「発祥の地・記念碑」分野は curated(F-03)が主源泉となる。
F-02 注記(同): 都 OD 文化財一覧 CSV は緯度・経度列を持つ(ジオコーディング不要)。
中央区該当は現状 1 件(八重洲・日本橋圏)のみで、本エリアでは 0 件でも正常。P6 の日本橋拡張で効く。
...
| N-05 | 公開リポジトリとして、コードは MIT LICENSE、データは CC BY 4.0(出典明記)の二層でライセンスを明示する。curated の summary は自著として CC BY に含める。ライセンス未配置の状態で public 化しない |
