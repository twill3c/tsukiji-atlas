...
| T-044 | F-06/N-03 | 描画スモーク(ヘッドレス Chromium、`npm run smoke`) | 地図コンテナ高さ >300px、マーカー数 = Feature 数、pageerror 0(loop_011 の高さ 0 バグ再発防止) |

## 統合(T-05x)— loop_007 で追加

| ID | 対応 | ケース | 期待 |
|---|---|---|---|
| T-051 | N-01/F-04/F-05 | フィクスチャから bronze 解析 → silver(名寄せ・era)→ gold(export+validate)を通し実行 | ネットワークなしで全層再現、qid 統合レコードが Gold 契約を満たし validate 合格 |

## 手動確認(自動化対象外)

- Q-06: summary の逐語一致なし(公開前に全件目視。チェック結果をループログに記録)
- 地図の実表示・地理院タイル帰属・色覚配慮(P4 完了時チェックリスト。実表示の機械検証部分は
  T-044 に半自動化済み — タイルの見た目・色覚配慮は引き続き目視)
