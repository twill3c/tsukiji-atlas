...
- **loop_001 の必須確認事項**(仮置きの検証): 中央区(東京都)の QID、記念碑・史跡系の
  クラス(Q4989906 monument / Q839954 archaeological site 等)のどれにヒットが集まるか、
  P131(行政区)と P625(座標)の充足率。確認前に本実装へ進まない
...
- 座標列がない場合、住所のジオコーディングは行わず curated 側で補完する(P1 で判断)
...
- 中央区 QID と記念碑系クラスの実態(loop_001 で解消)
- 都 OD CSV の座標有無 → ジオコーディング方針(loop_001 で解消)
